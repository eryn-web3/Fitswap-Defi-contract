...
// File: @fitswap/matrixswap-lib/contracts/token/BEP20/IBEP20.sol
...
// File: contracts/FitswapProfile.sol

pragma solidity ^0.6.0;

/** @title FitswapProfile.
@dev It is a contract for users to bind their address 
to a customizable profile by depositing a NFT.
*/
contract FitswapProfile is AccessControl, ERC721Holder {
    using Counters for Counters.Counter;
    using SafeBEP20 for IBEP20;
    using SafeMath for uint256;

    IBEP20 public ftsToken;
    ...
    uint256 public numberFtsToReactivate;
    uint256 public numberFtsToRegister;
    uint256 public numberFtsToUpdate;
    ...
    constructor(
        IBEP20 _ftsToken,
        uint256 _numberFtsToReactivate,
        uint256 _numberFtsToRegister,
        uint256 _numberFtsToUpdate
    ) public {
        ftsToken = _ftsToken;
        numberFtsToReactivate = _numberFtsToReactivate;
        numberFtsToRegister = _numberFtsToRegister;
        numberFtsToUpdate = _numberFtsToUpdate;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev To create a user profile. It sends the NFT to the contract
     * and sends FTS to burn address. Requires 2 token approvals.
     */
    function createProfile(
        ...
        // Transfer FTS tokens to this contract
        ftsToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberFtsToRegister
        );
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        // Transfer FTS token to this address
        ftsToken.safeTransferFrom(
            _msgSender(),
            address(this),
            numberFtsToUpdate
    ...
    /**
     * @dev Claim FTS to burn later.
     * Callable only by owner admins.
     */
    function claimFee(uint256 _amount) external onlyOwner {
        ftsToken.safeTransfer(_msgSender(), _amount);
    }
    ...
    /**
     * @dev Update the number of FTS to register
     * Callable only by owner admins.
     */
    function updateNumberFts(
        uint256 newNumberFtsToReactivate,
        uint256 _newNumberFtsToRegister,
        uint256 _newNumberFtsToUpdate
    ) external onlyOwner {
        numberFtsToReactivate = newNumberFtsToReactivate;
        numberFtsToRegister = _newNumberFtsToRegister;
        numberFtsToUpdate = _newNumberFtsToUpdate;
    }
