...
// added functionnalities by the fitswap's devs
//
...
contract SmartChef is Ownable {
    ...
    // The block number when FTS mining ends.
    uint256 public bonusEndBlock;
    // Deposited amount FTS in SmartChef
	...
	uint256 lpSupply = depositedFts;
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 ftsReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function deposit(uint256 _amount) public {
        ...
        if(_amount > 0) {
            ...
            if(pool.depositFeeBP > 0){
                ...
                depositedFts = depositedFts.add(_amount).sub(depositFee).sub(transferTax);
            }else{
                uint256 transferTax = _amount.mul(fts.transferTaxRate()).div(10000);
                user.amount = user.amount.add(_amount).sub(transferTax);
                depositedFts = depositedFts.add(_amount).sub(transferTax);
            }
        }        
    ...
    // Withdraw FTS tokens from STAKING.
