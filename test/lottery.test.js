...
contract('Lottery', (accounts) => {
    it('init', async () => {
        ...
        this.fts = await MockBEP20.deployed();
        ...
        await this.fts.transfer(bob, '2000', { from: minter });
        await this.fts.transfer(alice, '2000', { from: minter });
        await this.fts.transfer(carol, '2000', { from: minter });
    });
    ...
    it('test', async () => {
        ...
        carol=accounts[3];
        dev=accounts[4];
        await this.fts.approve(this.lotteryProxyAddress, '1000', { from: alice });
        await this.fts.approve(this.lotteryProxyAddress, '1000', { from: bob });
        ...
        assert.equal((await this.fts.balanceOf(this.lotteryProxyAddress)).toString(), '418');
